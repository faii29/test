...
<script>
  // Pop-in countdown boxes one-by-one
  function animateCountdownBoxes() {
    const boxes = document.querySelectorAll('.count-box');
    boxes.forEach((box, index) => {
      setTimeout(() => {
        box.style.animation = `popIn 0.6s ease forwards`;
      }, index * 500); // delay each by 500ms
    });
  }

  // Show boxes after small delay (simulate page load or button trigger)
